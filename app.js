...
async function uploadToRoblox() { if (!converted) return; const downloadResponse = await fetch(converted.downloadUrl, { credentials: 'include' }); if (!downloadResponse.ok) return setNotice('Hasil audio tidak dapat dibaca.', 'error'); const blob = await downloadResponse.blob(); const form = new FormData(); form.append('audio', blob, converted.name); form.append('name', converted.name); $('#uploadButton').disabled = true; setNotice('Mengirim asset ke Roblox dan menunggu status moderasi...'); try { const response = await fetch('/api/roblox/upload-audio', { method: 'POST', credentials: 'include', body: form }); let data = await response.json().catch(() => ({})); if (response.status === 202 && data.operationId) { for (let attempt = 0; attempt < 40; attempt += 1) { await new Promise((resolve) => window.setTimeout(resolve, 3000)); const pollResponse = await fetch(`/api/roblox/operations/${encodeURIComponent(data.operationId)}`, { credentials: 'include' }); data = await pollResponse.json().catch(() => ({})); if (pollResponse.ok && data.success && data.assetId) break; if (pollResponse.status !== 202) throw new Error(data.error || 'Roblox gagal memproses audio.'); if (attempt === 39) throw new Error(data.error || 'Roblox masih memproses audio.'); } } if (!response.ok && !data.success) throw new Error(data.error || 'Upload gagal'); if (data.success !== true || !data.assetId) throw new Error(data.error || 'Roblox belum mengembalikan Asset ID final.'); setNotice(`Asset ID: ${data.assetId}`, 'success'); $('#copyAssetButton').dataset.id = data.assetId; show('#copyAssetButton', true); loadHistory(); } catch (error) { setNotice(error.message, 'error'); } finally { $('#uploadButton').disabled = false; } }
